...
use crate::core::tweet::{Tweet, TweetContent};
...
// TODO: content_text and context_image
/// Compute the path of the tweet content
/// /tweets/{hash}/content
fn tweet_content_text_path(tweet_id: &u64) -> Result<OwnedPath> {
    tweet_field_path(tweet_id, "/content_text")
}

fn tweet_content_img_path(tweet_id: &u64) -> Result<OwnedPath> {
    tweet_field_path(tweet_id, "/content_img")
}
...
/// Stores bytes at a given path
/// option of tag: u8
fn store_bytes<'a, R: Runtime>(host: &mut R, path: &OwnedPath, data: &'a [u8]) -> Result<&'a [u8]> {
    host.store_write(path, data, 0)
        .map_err(Error::from)
        .map(|_| data)
}
...
pub fn store_tweet<'a, R: Runtime>(
    ...
    let content_text_path = tweet_content_text_path(tweet_id)?;
    let content_img_path = tweet_content_img_path(tweet_id)?;

    match content {
        TweetContent::Image(img) => {
            let _ = store_bytes(host, &content_img_path, img)?;
        }
        TweetContent::Text(text) => {
            let _ = store_string(host, &content_text_path, text)?;
        }
    };
...
pub fn read_tweet<R: Runtime>(host: &mut R, tweet_id: &u64) -> Result<Option<Tweet>> {
    ...
    let content_text = read_string(host, &content_text_path)?;
    let content_img = read_string(host, &content_img_path)?;

    let content = match (content_text, content_img) {
        (Some(text), _) => Some(TweetContent::Text(text)),
        (_, Some(img)) => {
            let decoded = hex::decode(img).map_err(Error::from)?;
            Some(TweetContent::Image(decoded))
        }
        _ => None,
    };
